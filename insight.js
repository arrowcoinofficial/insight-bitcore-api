...
process.env.NODE_ENV = process.env.NODE_ENV || 'livenet';
...
\tINSIGHT_IGNORE_CACHE (Ignore Cache):  %s\n\
 # BicoinDarkd Connection configuration:\n\
...
server.listen(config.port, function() {
  logger.info('Explore Arrowcoin server listening on port %d in %s mode', server.address().port, process.env.NODE_ENV);
});
